...
## 採集並寫入 `printer_metrics`

`scripts/collect_metrics.go` 會讀取 `printers` 表中的 IP 清單，透過 SNMP v2c 抓取印表機累積頁數，並以 `(printer_id, recorded_at)` 為唯一鍵寫入或更新 `printer_metrics`。

預設使用 Printer-MIB 的 `prtMarkerLifeCount`：
...
go run .\scripts\collect_metrics.go
```
...
```powershell
# 只測試，不寫入資料庫
go run .\scripts\collect_metrics.go -dry-run

# 指定寫入日期
go run .\scripts\collect_metrics.go -date 2026-05-08

# 只採集單一 printers.id
go run .\scripts\collect_metrics.go -printer-id 1
```

如果某些機型有獨立列印、影印、掃描、傳真 OID，可以用環境變數覆寫：

```powershell
$env:PRINTER_METRICS_TOTAL_OID="<total_count_oid>"
$env:PRINTER_METRICS_PRINT_OID="<print_count_oid>"
$env:PRINTER_METRICS_COPY_OID="<copy_count_oid>"
$env:PRINTER_METRICS_SCAN_OID="<scan_total_oid>"
$env:PRINTER_METRICS_FAX_OID="<fax_count_oid>"
```

若未設定 `PRINT/COPY/SCAN/FAX` OID，腳本會用總頁數作為 `print_count`，並沿用前一次的 `copy_count`、`scan_total`、`fax_count`，避免把既有累積欄位歸零。
...
Windows 11 沒有內建 Unix cron，建議使用「工作排程器」建立每半小時執行一次的排程工作，用來先寫入 `printer_metrics`，再更新 `public/data.json`。
...
5. 在「動作」頁籤選擇「新增」：
   ...
   ```powershell
   -NoProfile -ExecutionPolicy Bypass -Command "$env:SUPABASE_DB_CONNECTION='<SUPABASE_DB_CONNECTION>'; $env:SNMP_COMMUNITY='public'; Set-Location 'D:\myprograms\printerinfo'; go run .\scripts\collect_metrics.go; if ($LASTEXITCODE -eq 0) { go run .\scripts\main.go }"
   ```
...
$taskRun = 'powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$env:SUPABASE_DB_CONNECTION=''<SUPABASE_DB_CONNECTION>''; $env:SNMP_COMMUNITY=''public''; Set-Location ''D:\myprograms\printerinfo''; go run .\scripts\collect_metrics.go; if ($LASTEXITCODE -eq 0) { go run .\scripts\main.go }"'
